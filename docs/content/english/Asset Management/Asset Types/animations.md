...
After exporting your animation, referencing it in the game is done from files starting with animations_, such as animations_combat.xml. An animation requires these attributes: id, duration, anim_data_name, source1, source2. anim_data_name references the exported file, and source1 and source2 are the key frames for the animation to play between, which could be even backwards. There is no frame/sec limitation in the engine, and it just maps the number of frames to the given duration by blending.
...
Non-agent animations are not mapped as actions. For those, you just need to add the animation to one of the animation XML files and then start using it in the code.
